...
public class ListViewAnimationInterfaceSettings extends SettingsPreferenceFragment implements
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        int listviewDuration = Settings.XOPLAX.getInt(mResolver,
                Settings.XOPLAX.LISTVIEW_DURATION, 0);
    ...
    private void resetAllSettings() {
        ...
        Settings.XOPLAX.putString(getContentResolver(),
                Settings.XOPLAX.LISTVIEW_ANIMATION_EXCLUDED_APPS, "");
    ...
    public boolean onPreferenceChange(Preference preference, Object objValue) {
        if (preference == mListViewAnimation) {
            ...
            Settings.XOPLAX.putInt(mResolver, Settings.XOPLAX.LISTVIEW_ANIMATION, val);
            mListViewAnimation.setSummary(getListAnimationName(val));
        } else if (preference == mListViewCache) {
            int val = Integer.parseInt((String) objValue);
            Settings.XOPLAX.putInt(mResolver, Settings.XOPLAX.LISTVIEW_ANIMATION_CACHE, val);
            mListViewCache.setSummary(getListCacheName(val));
        } else if (preference == mListViewInterpolator) {
            int val = Integer.parseInt((String) objValue);
            Settings.XOPLAX.putInt(mResolver, Settings.XOPLAX.LISTVIEW_INTERPOLATOR, val);
            ...
            Settings.XOPLAX.putInt(mResolver,
                    Settings.XOPLAX.LISTVIEW_DURATION,
                    val);
        } else {
            return false;
        }
    ...
    private void setProperVal(Preference preference, int val) {
        String mString = "";
        if (preference == mListViewAnimation) {
            mString = Settings.XOPLAX.LISTVIEW_ANIMATION;
        } else if (preference == mListViewCache) {
            mString = Settings.XOPLAX.LISTVIEW_ANIMATION_CACHE;
        } else if (preference == mListViewInterpolator) {
            mString = Settings.XOPLAX.LISTVIEW_INTERPOLATOR;
        } else if (preference == mListViewDuration) {
            mString = Settings.XOPLAX.LISTVIEW_DURATION;
        }

        Settings.XOPLAX.putInt(mContext.getContentResolver(), mString, val);
    }
    ...
    private String getProperVal(Preference preference) {
        ...
        return Settings.XOPLAX.getString(mContext.getContentResolver(), mString);
    }
    ...
    private void storeExcludedApps(Set<String> values) {
        ...
        Settings.XOPLAX.putString(getContentResolver(),
                Settings.XOPLAX.LISTVIEW_ANIMATION_EXCLUDED_APPS, builder.toString());
    }
