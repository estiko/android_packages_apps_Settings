...
public class SystemAnimationInterfaceSettings extends SettingsPreferenceFragment implements
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        mAnimNoOverride.setChecked(Settings.XOPLAX.getInt(mResolver,
        ...
        int defaultDuration = Settings.XOPLAX.getInt(mResolver,
    ...
    public boolean onPreferenceChange(Preference preference, Object objValue) {
        if (preference == mAnimNoOverride) {
            ...
            Settings.XOPLAX.putInt(mResolver, Settings.XOPLAX.ANIMATION_CONTROLS_NO_OVERRIDE, value ? 1 : 0);
        } else if (preference == mActivityOpenPref) {
            int val = Integer.parseInt((String) objValue);
            Settings.XOPLAX.putInt(mResolver,
                    Settings.XOPLAX.ACTIVITY_ANIMATION_CONTROLS[0], val);
            mActivityOpenPref.setSummary(getProperSummary(mActivityOpenPref));
        } else if (preference == mActivityClosePref) {
            int val = Integer.parseInt((String) objValue);
            Settings.XOPLAX.putInt(mResolver,
                    Settings.XOPLAX.ACTIVITY_ANIMATION_CONTROLS[1], val);
            mActivityClosePref.setSummary(getProperSummary(mActivityClosePref));
        } else if (preference == mTaskOpenPref) {
            int val = Integer.parseInt((String) objValue);
            Settings.XOPLAX.putInt(mResolver,
                    Settings.XOPLAX.ACTIVITY_ANIMATION_CONTROLS[2], val);
            mTaskOpenPref.setSummary(getProperSummary(mTaskOpenPref));
        } else if (preference == mTaskClosePref) {
            int val = Integer.parseInt((String) objValue);
            Settings.XOPLAX.putInt(mResolver,
                    Settings.XOPLAX.ACTIVITY_ANIMATION_CONTROLS[3], val);
            mTaskClosePref.setSummary(getProperSummary(mTaskClosePref));
        } else if (preference == mTaskMoveToFrontPref) {
            int val = Integer.parseInt((String) objValue);
            Settings.XOPLAX.putInt(mResolver,
                    Settings.XOPLAX.ACTIVITY_ANIMATION_CONTROLS[4], val);
            mTaskMoveToFrontPref.setSummary(getProperSummary(mTaskMoveToFrontPref));
        } else if (preference == mTaskMoveToBackPref) {
            int val = Integer.parseInt((String) objValue);
            Settings.XOPLAX.putInt(mResolver,
                    Settings.XOPLAX.ACTIVITY_ANIMATION_CONTROLS[5], val);
            mTaskMoveToBackPref.setSummary(getProperSummary(mTaskMoveToBackPref));
        } else if (preference == mWallpaperOpen) {
            int val = Integer.parseInt((String) objValue);
            Settings.XOPLAX.putInt(mResolver,
                    Settings.XOPLAX.ACTIVITY_ANIMATION_CONTROLS[6], val);
            mWallpaperOpen.setSummary(getProperSummary(mWallpaperOpen));
        } else if (preference == mWallpaperClose) {
            int val = Integer.parseInt((String) objValue);
            Settings.XOPLAX.putInt(mResolver,
                    Settings.XOPLAX.ACTIVITY_ANIMATION_CONTROLS[7], val);
            mWallpaperClose.setSummary(getProperSummary(mWallpaperClose));
        } else if (preference == mWallpaperIntraOpen) {
            int val = Integer.parseInt((String) objValue);
            Settings.XOPLAX.putInt(mResolver,
                    Settings.XOPLAX.ACTIVITY_ANIMATION_CONTROLS[8], val);
            mWallpaperIntraOpen.setSummary(getProperSummary(mWallpaperIntraOpen));
        } else if (preference == mWallpaperIntraClose) {
            int val = Integer.parseInt((String) objValue);
            Settings.XOPLAX.putInt(mResolver,
                    Settings.XOPLAX.ACTIVITY_ANIMATION_CONTROLS[9], val);
            mWallpaperIntraClose.setSummary(getProperSummary(mWallpaperIntraClose));
        } else if (preference == mAnimationDuration) {
            int val = ((Integer)objValue).intValue();
            Settings.XOPLAX.putInt(mResolver,
                    Settings.XOPLAX.ANIMATION_CONTROLS_DURATION,
                    val);
        } else {
            return false;
        }
    ...
    private void setProperVal(Preference preference, int val) {
        ...
        if (preference == mActivityOpenPref) {
            mString = Settings.XOPLAX.ACTIVITY_ANIMATION_CONTROLS[0];
        } else if (preference == mActivityClosePref) {
            mString = Settings.XOPLAX.ACTIVITY_ANIMATION_CONTROLS[1];
        } else if (preference == mTaskOpenPref) {
            mString = Settings.XOPLAX.ACTIVITY_ANIMATION_CONTROLS[2];
        } else if (preference == mTaskClosePref) {
            mString = Settings.XOPLAX.ACTIVITY_ANIMATION_CONTROLS[3];
        } else if (preference == mTaskMoveToFrontPref) {
            mString = Settings.XOPLAX.ACTIVITY_ANIMATION_CONTROLS[4];
        } else if (preference == mTaskMoveToBackPref) {
            mString = Settings.XOPLAX.ACTIVITY_ANIMATION_CONTROLS[5];
        } else if (preference == mWallpaperOpen) {
            mString = Settings.XOPLAX.ACTIVITY_ANIMATION_CONTROLS[6];
        } else if (preference == mWallpaperClose) {
            mString = Settings.XOPLAX.ACTIVITY_ANIMATION_CONTROLS[7];
        } else if (preference == mWallpaperIntraOpen) {
            mString = Settings.XOPLAX.ACTIVITY_ANIMATION_CONTROLS[8];
        } else if (preference == mWallpaperIntraClose) {
            mString = Settings.XOPLAX.ACTIVITY_ANIMATION_CONTROLS[9];
        }

        Settings.XOPLAX.putInt(mResolver, mString, val);
    }
    ...
    private String getProperSummary(Preference preference) {
        ...
        String mNum = Settings.XOPLAX.getString(mResolver, mString);
    ...
    private String getProperVal(Preference preference) {
        ...
        return Settings.XOPLAX.getString(mResolver, mString);
    }
