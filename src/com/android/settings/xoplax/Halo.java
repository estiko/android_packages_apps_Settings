...
public class Halo extends SettingsPreferenceFragment
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        mHaloHide.setChecked(Settings.XOPLAX.getInt(mContext.getContentResolver(),
                Settings.XOPLAX.HALO_HIDE, 0) == 1);

        int isLowRAM = (!ActivityManager.isLowRamDeviceStatic()) ? 0 : 1;
        mHaloPause = (CheckBoxPreference) prefSet.findPreference(KEY_HALO_PAUSE);
        mHaloPause.setChecked(Settings.XOPLAX.getInt(mContext.getContentResolver(),
                Settings.XOPLAX.HALO_PAUSE, isLowRAM) == 1);

        mHaloSize = (ListPreference) prefSet.findPreference(KEY_HALO_SIZE);
        try {
            float haloSize = Settings.XOPLAX.getFloat(mContext.getContentResolver(),
        ...
        mHaloMsgBox.setChecked(Settings.XOPLAX.getInt(mContext.getContentResolver(),
                Settings.XOPLAX.HALO_MSGBOX, 1) == 1);

        mHaloUnlockPing = (CheckBoxPreference) prefSet.findPreference(KEY_HALO_UNLOCK_PING);
        mHaloUnlockPing.setChecked(Settings.XOPLAX.getInt(mContext.getContentResolver(),
                Settings.XOPLAX.HALO_UNLOCK_PING, 0) == 1);

        mHaloNotifyCount = (ListPreference) prefSet.findPreference(KEY_HALO_NOTIFY_COUNT);
        try {
            int haloCounter = Settings.XOPLAX.getInt(mContext.getContentResolver(),
        ...
        try {
            int haloMsgAnimation = Settings.XOPLAX.getInt(mContext.getContentResolver(),
    ...
    public boolean onPreferenceTreeClick(PreferenceScreen preferenceScreen, Preference preference) {
        if (preference == mHaloHide) {
            Settings.XOPLAX.putInt(mContext.getContentResolver(),
                    Settings.XOPLAX.HALO_HIDE, mHaloHide.isChecked()
                    ? 1 : 0);
        } else if (preference == mHaloPause) {
            Settings.XOPLAX.putInt(mContext.getContentResolver(),
                    Settings.XOPLAX.HALO_PAUSE, mHaloPause.isChecked()
                    ? 1 : 0);
        } else if (preference == mHaloMsgBox) {
            Settings.XOPLAX.putInt(mContext.getContentResolver(),
                    Settings.XOPLAX.HALO_MSGBOX, mHaloMsgBox.isChecked()
                    ? 1 : 0);
        } else if (preference == mHaloUnlockPing) {
            Settings.XOPLAX.putInt(mContext.getContentResolver(),
                    Settings.XOPLAX.HALO_UNLOCK_PING, mHaloUnlockPing.isChecked()
                    ? 1 : 0);
        }
    ...
    public boolean onPreferenceChange(Preference preference, Object newValue) {
        if (preference == mHaloSize) {
            ...
            Settings.XOPLAX.putFloat(getActivity().getContentResolver(),
                    Settings.XOPLAX.HALO_SIZE, haloSize);
            return true;
            ...
            Settings.XOPLAX.putInt(getActivity().getContentResolver(),
                    Settings.XOPLAX.HALO_MSGBOX_ANIMATION, haloMsgAnimation);
            return true;
        } else if (preference == mHaloNotifyCount) {
            int haloNotifyCount = Integer.valueOf((String) newValue);
            Settings.XOPLAX.putInt(getActivity().getContentResolver(),
                    Settings.XOPLAX.HALO_NOTIFY_COUNT, haloNotifyCount);
            return true;
        }
        return false;
    }
